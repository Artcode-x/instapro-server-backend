...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") { 
    ...
    // токен нужен для проверки стоит true или false в лайках. 
    // если не передаем токен, в isLike всегда будет false

    return res.status(200).json({
      posts: posts.map((post) => mapPost(post, user)), 
      // перезапись происходит через mapPost (в libs) - она определяет где какой ключ
      // метод map исп-ся чтобы перезаписать все данные
      // форматирование данных сервером для нас
    });
  }
  ...
  try {
    if (req.method === "POST") {
      try {
        ...
        // разбиваем строку JSON, чтобы было легче с ней работать
        ...
        await addPost({ // await исп-ся для ожидания окончания promise. 
          //ожидаем ответа от ф-ии addPost / как promise.
          user,
          description,
          imageUrl,
          key,
